...
public class DependsOnPropertyGenerator : Generators.SingleClassGenerator
{
    ...
    protected override void GenerateInternal()
    {
        ...
        foreach (var i in slavesForMaster)
        {
            ...
            var value     = string.Join(",", sink.Distinct()); // don't use CommaJoin()
    ...
    {
        var wr = CsCodeWriter.Create<DependsOnPropertyGenerator>();
        ...
        foreach (var byAffectedFieldList in names.GroupBy(a => a.Value.FieldsList))
        {
            ...
            {
                var caseTxt = GetCaseTxt(pair, --left == 0);
                wr.WriteLine(caseTxt);
                continue;

                string GetCaseTxt(KeyValuePair<string, Info> xpair, bool last)
                {
                    var txt = $"case nameof({xpair.Key}):";
                    if (!last) return txt;
                    var value = GetConstCode();
                    txt += $" return {value};";
                    if (value.StartsWith("\"", StringComparison.Ordinal))
                        return txt;
                    txt += $" // {xpair.Value.FieldsList}";
                    return txt;
                }
        ...
        myClass.AddMethod(GetDependentPropertiesMethodName, CsType.StringNullable)
