...
namespace iSukces.Code.Translations
{
    ...
    /// </summary>
    public sealed class CreateLiteLocalTextSources_Request
    {
        public CreateLiteLocalTextSources_Request(string key, string originalText, string? hint = null)
        {
        ...
        public static implicit operator CreateLiteLocalTextSources_Request(string x)
        {
            var parts = x.Split('|');
            if (parts.Length < 2)
                throw new Exception("Invalid text");
            return new CreateLiteLocalTextSources_Request(parts[0], parts[1], parts.Length > 2 ? parts[2] : null);
        }
